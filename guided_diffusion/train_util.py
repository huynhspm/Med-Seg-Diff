...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while (
            ...
            try:
                    batch, cond, tumor, image_path  = next(data_iter)
    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                if not rate:
                    filename = f"savedmodel{(self.epoch):02d}.pt"
                else:
                    filename = f"emasavedmodel_{rate}_{(self.epoch):02d}.pt"
        ...
        if dist.get_rank() == 0:
            with bf.BlobFile(
                bf.join(get_blob_logdir(), f"optsavedmodel{(self.epoch):02d}.pt"),
                "wb",
            ) as f:
