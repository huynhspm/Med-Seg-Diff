...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if high_way:
            features = 32
            self.hwm = Generic_UNet(self.in_channels - 1, features, 1, 5, image_size=self.image_size)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        hs = []
        a = timestep_embedding(timesteps, self.model_channels)
        emb = self.time_embed(a)
...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            if d != num_pool - 1:
                self.ffparser.append(FFParser(output_features, self.image_size // (2 **(d+1)), self.image_size // (2 **(d+2))+1))
